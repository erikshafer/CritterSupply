Feature: Product Content Management
  As a copy writer
  I want to update product descriptions and display names in the Admin Portal
  ...
  Scenario: Copy writer cannot submit an empty description
    ...
    And I click Save
    Then the Admin Portal shows a validation error "Description cannot be empty"
  ...
  Scenario: Copy writer cannot submit a description exceeding the maximum length
    ...
    And I click Save
    Then the Admin Portal shows a validation error "Description cannot exceed 5000 characters"
    And no changes are sent to the Product Catalog BC
