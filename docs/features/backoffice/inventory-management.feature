Feature: Inventory Management
  ...
  I want to adjust stock levels and receive inbound goods through the Backoffice
  ...
  Scenario: Receiving stock clears an active low-stock alert when stock exceeds threshold
    ...
    And connected Backoffice clients receive a real-time alert dismissal notification
  ...
  Scenario: Warehouse clerk adjusts inventory for a cycle count discrepancy
    ...
    Then the Backoffice shows a success confirmation with the new available quantity of 3
  ...
  Scenario: Inventory adjustment that drops below low-stock threshold triggers an alert
    ...
    And the Backoffice shows a low-stock alert notification for SKU "CAT-TOY-007"
  ...
  Scenario: Warehouse clerk cannot submit a receive with zero quantity
    ...
    And I click Receive
    Then the Backoffice shows a validation error "Quantity must be greater than zero"
