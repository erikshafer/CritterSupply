Feature: Backoffice Live Dashboard
  ...
  Background:
    Given the Backoffice is running with SignalR enabled
    And I am connected to the Backoffice SignalR hub at /hub/backoffice

  Scenario: Executive sees live revenue counter update when an order is placed
    Given I am logged in to the Backoffice as an "Executive"
    ...
    And the Backoffice SignalR hub pushes a "LiveMetricUpdated" message to the "role:executive" group
  ...
  Scenario: Operations manager sees a real-time alert when a payment fails
    Given I am logged in to the Backoffice as an "OperationsManager"
    When the Payments BC publishes a "PaymentFailed" event for order "order-uuid-001"
    Then the Backoffice SignalR hub pushes an "AlertRaised" message to the "role:operations" group
  ...
  Scenario: Warehouse clerk sees a low-stock alert in real time
    ...
    Then the Backoffice SignalR hub pushes a "LowStockAlertRaised" message to the "role:warehouseclerk" group
  ...
  Scenario: Executive can export a sales report as CSV
    ...
    Then the Backoffice downloads a CSV file containing:
      | Column           |
      | Date             |
      | SKU              |
      | Units Sold       |
  ...
  Scenario: SignalR alerts are role-scoped — executives do not receive inventory alerts
    ...
    Then the Backoffice SignalR hub does NOT push the "LowStockAlertRaised" message to the "role:executive" group
  ...
  Scenario: Unauthenticated connection to the admin SignalR hub is rejected
    ...
    When I attempt to connect to the Backoffice SignalR hub at /hub/backoffice
  ...
  Scenario: Operations manager receives an alert when a saga has been stuck for over 1 hour
    ...
    Then the Backoffice pushes a "StuckSagaAlertRaised" notification to the "role:operations" group
