...
Feature: Pricing Admin
  ...
  Background:
    ...
    And stub catalog has product "DEMO-001" with name "Cat Food Premium"
    And stub pricing client has product "DEMO-001" with current price "$19.99"

  Scenario: Pricing Manager can set base price
    Given admin user "PricingAdmin" exists with email "pricing@example.com" and role "PricingManager"
    And I am logged in as "pricing@example.com"
    When I navigate to the price edit page for SKU "DEMO-001"
  ...
  Scenario: SystemAdmin can set prices
    Given admin user "SystemAdmin" exists with email "admin@example.com" and role "system-admin"
