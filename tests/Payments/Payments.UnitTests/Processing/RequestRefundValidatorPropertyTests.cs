...
/// <summary>
/// Property-based tests for RequestRefund validation.
/// </summary>
public class RequestRefundValidatorPropertyTests
{
    private readonly RequestRefund.RequestRefundValidator _validator = new();

    /// <summary>
    /// **Feature: payment-processing, Property 6: Refund validation rejects invalid requests**
    ///
    /// *For any* RequestRefund command where the PaymentId is empty or the amount is less than
    /// or equal to zero, the refund SHALL be rejected.
    ///
    /// **Validates: Requirements 5.1, 5.3**
    /// </summary>
    [Property(MaxTest = 100, Arbitrary = [typeof(InvalidRefundRequestArbitrary)])]
    public bool Validation_Rejects_Invalid_Refund_Requests(RequestRefund command)
    {
    ...
    public bool Validation_Rejects_Empty_PaymentId(RequestRefund command)
    {
        ...
        var hasPaymentIdError = result.Errors.Any(e =>
            e.PropertyName == nameof(RequestRefund.PaymentId));
    ...
    public bool Validation_Rejects_Invalid_Refund_Amounts(RequestRefund command)
    {
        ...
        var hasAmountError = result.Errors.Any(e =>
            e.PropertyName == nameof(RequestRefund.Amount));
...
/// <summary>
/// Arbitrary that generates RequestRefund commands with invalid data (empty PaymentId or invalid amount).
/// </summary>
public static class InvalidRefundRequestArbitrary
{
    public static Arbitrary<RequestRefund> RequestRefund()
    {
        ...
        var emptyPaymentIdGen = Gen.Constant(Guid.Empty)
            .SelectMany(paymentId => ArbMap.Default.GeneratorFor<Guid>()
                .SelectMany(orderId => Gen.Choose(1, 100000).Select(cents => (decimal)cents / 100)
                    .Select(amount => new Payments.Processing.RequestRefund(paymentId, orderId, amount))));
...
/// <summary>
/// Arbitrary that generates RequestRefund commands with empty PaymentId.
/// </summary>
public static class EmptyPaymentIdRefundArbitrary
{
    public static Arbitrary<RequestRefund> RequestRefund()
    {
        var gen = ArbMap.Default.GeneratorFor<Guid>()
            .SelectMany(orderId => Gen.Choose(1, 100000).Select(cents => (decimal)cents / 100)
                .Select(amount => new Payments.Processing.RequestRefund(Guid.Empty, orderId, amount)));
...
/// <summary>
/// Arbitrary that generates RequestRefund commands with invalid amounts (zero or negative).
/// </summary>
