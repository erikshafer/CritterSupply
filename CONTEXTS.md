...
| Marketplaces | ↔ bidirectional | Publishes `ListingApproved` to trigger adapter submission; receives `MarketplaceListingActivated` (→ Live) and `MarketplaceSubmissionRejected` (→ Ended) |
...
**Key decisions:** Document store for configuration data ([ADR 0048](../decisions/0048-marketplace-document-entity-design.md)). Category mapping ownership in Marketplaces BC ([ADR 0049](../decisions/0049-category-mapping-ownership.md)). `ProductSummaryView` ACL isolates Marketplaces from Listings message payload ([ADR 0050](../decisions/0050-marketplaces-product-summary-acl.md)). `IVaultClient` abstraction: `DevVaultClient` in Development, `EnvironmentVaultClient` in production ([ADR 0051](../decisions/0051-vault-implementation-strategy.md)). `IMarketplaceAdapter` interface with three production implementations: `AmazonMarketplaceAdapter` (LWA OAuth 2.0, [ADR 0052](../decisions/0052-amazon-spapi-authentication.md)), `WalmartMarketplaceAdapter` (client credentials, [ADR 0053](../decisions/0053-walmart-marketplace-api-authentication.md)), `EbayMarketplaceAdapter` (refresh token, two-step submit, [ADR 0054](../decisions/0054-ebay-sell-api-authentication.md)). `UseRealAdapters` config flag controls stub vs. real adapter registration — stubs are always active in Development/CI. Walmart-only submission status polling via per-submission scheduled messages ([ADR 0055](../decisions/0055-submission-status-polling-architecture.md)). Polly resilience (retry + circuit breaker) registered on all three adapter `HttpClient` pipelines via `Microsoft.Extensions.Http.Resilience` ([ADR 0056](../decisions/0056-marketplace-adapter-resilience-patterns.md)). Walmart deactivation identifier design resolved — `externalListingId` encodes `wmrt-{sku}` (not feed ID); `DeactivateListingAsync` submits RETIRE_ITEM feed ([ADR 0057](../decisions/0057-walmart-deactivation-identifier-design.md)). All three adapters have real `CheckSubmissionStatusAsync` implementations: Amazon checks SP-API listing status (BUYABLE = live, 404 = pending), eBay checks offer status (PUBLISHED = live, UNPUBLISHED = orphaned draft surfaced as failed), Walmart uses feed status polling.
