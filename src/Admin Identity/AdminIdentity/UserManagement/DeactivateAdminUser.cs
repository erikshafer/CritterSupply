using AdminIdentity.Identity;
using FluentValidation;
using Microsoft.EntityFrameworkCore;

namespace AdminIdentity.UserManagement;

/// <summary>
/// Command to deactivate an admin user account.
/// Deactivated users cannot log in but their data is retained for audit purposes.
/// Only SystemAdmin role can deactivate users.
/// </summary>
public sealed record DeactivateAdminUser(Guid UserId, string Reason)
{
    public sealed class DeactivateAdminUserValidator : AbstractValidator<DeactivateAdminUser>
    {
        public DeactivateAdminUserValidator()
        {
            RuleFor(x => x.UserId)
                .NotEmpty()
                .WithMessage("User ID is required.");

            RuleFor(x => x.Reason)
                .NotEmpty()
                .MaximumLength(500)
                .WithMessage("Deactivation reason is required (max 500 characters).");
        }
    }
}

/// <summary>
/// Response returned when a user is deactivated.
/// </summary>
public sealed record DeactivateAdminUserResponse(
    Guid UserId,
    string Email,
    string Reason,
    DateTimeOffset DeactivatedAt);

/// <summary>
/// Handler for deactivating admin users.
/// Invalidates refresh tokens to force logout.
/// </summary>
public static class DeactivateAdminUserHandler
{
    public static async Task<AdminUser?> Load(
        DeactivateAdminUser command,
        AdminIdentityDbContext db,
        CancellationToken ct)
    {
        return await db.Users
            .Where(u => u.Id == command.UserId)
            .FirstOrDefaultAsync(ct);
    }

    public static async Task<(DeactivateAdminUserResponse?, ProblemDetails?)> Handle(
        DeactivateAdminUser command,
        AdminUser? user,
        AdminIdentityDbContext db,
        CancellationToken ct)
    {
        // User not found
        if (user is null)
        {
            return (null, new ProblemDetails
            {
                Detail = $"Admin user with ID '{command.UserId}' was not found.",
                Status = StatusCodes.Status404NotFound
            });
        }

        // Idempotent: user already deactivated
        if (user.Status == AdminUserStatus.Deactivated)
        {
            return (new DeactivateAdminUserResponse(
                user.Id,
                user.Email,
                user.DeactivationReason ?? command.Reason,
                user.DeactivatedAt!.Value), null);
        }

        // Deactivate user
        user.Status = AdminUserStatus.Deactivated;
        user.DeactivatedAt = DateTimeOffset.UtcNow;
        user.DeactivationReason = command.Reason;

        // Invalidate refresh token to force logout
        user.RefreshToken = null;
        user.RefreshTokenExpiresAt = null;

        await db.SaveChangesAsync(ct);

        var response = new DeactivateAdminUserResponse(
            user.Id,
            user.Email,
            user.DeactivationReason,
            user.DeactivatedAt.Value);

        return (response, null);
    }
}
